    ...
    <div style={S.root}>
      ...
      <nav style={S.nav}>
        {[
          ...
          <button key={id}
            style={{ ...S.navBtn, ...(view === id || (view === "quiz" && id === "study") ? S.navBtnActive : {}) }}
            onClick={() => setView(id)}>
            <Icon d={icon} size={16} />
            <span>{label}</span>
          </button>
      ...
      <main style={S.main}>
        ...
        {view === "manage" && (
          <ManageView cards={cards} selected={selected} setSelected={setSelected}
            onDelete={deleteCard} onDeleteSelected={deleteSelected} onDeleteAll={deleteAll} />
        )}
      </main>

      {toast && (
        <div style={{ ...S.toast, ...(toast.type === "warn" ? S.toastWarn : S.toastOk) }} className="toast-in">
          {toast.msg}
        </div>
      )}
    </div>
...
function StudyView({ cards, dueCards, onStudy }) {
  const upcoming = cards.filter(c => !isDueToday(c))
  ...
  return (
    <div style={S.studyWrap}>
      <div style={S.heroCard}>
        ...
        {dueCards.length > 0
          ? <button style={S.startBtn} onClick={onStudy} className="pulse-btn">学習を始める →</button>
          : <div style={S.doneMsg}>🎉 今日の問題は全て完了！</div>
        }
      </div>
          ...
          {upcoming.map(c => {
            ...
            return (
              <div key={c.id} style={S.upcomingRow}>
                <div style={S.upcomingThumb}>
                  {c.questionImage
                    ? <img src={c.questionImage} style={S.thumbImg} alt="" />
                    : <Icon d={icons.img} size={14} />}
                </div>
                <div style={S.upcomingTitle}>{c.answer.slice(0, 30)}{c.answer.length > 30 ? "..." : ""}</div>
  ...
  if (!card) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px" }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: "#f1f5f9", marginBottom: 8 }}>全問完了！</div>
        <div style={{ fontSize: 14, color: "#64748b", marginBottom: 24 }}>お疲れ様でした</div>
        <button style={S.startBtn} onClick={onDone}>トップに戻る</button>
      </div>
    );
  }
  ...
  return (
    <div style={S.quizWrap}>
      <div style={S.progressBar}>
        <div style={{ ...S.progressFill, width: `${(idx / dueCards.length) * 100}%` }} />
      </div>
      <div style={S.progressLabel}>{idx + 1} / {dueCards.length}</div>

      <div style={S.quizCard} className="card-appear">
        <div style={S.quizSection}>
          <div style={S.quizSectionLabel}>問題</div>
          {card.questionImage
            ? <img src={card.questionImage} style={S.quizImg} alt="question" />
            : <div style={S.noImg}>📷 画像なし</div>}
          {card.questionText && <div style={S.quizText}>{card.questionText}</div>}
        </div>

        {!revealed ? (
          <button style={S.revealBtn} onClick={() => setRevealed(true)} className="reveal-btn">
            答えを見る
          </button>
        ) : (
          <div className="answer-reveal">
            <div style={S.answerBox}>
              ...
              <div style={S.answerText}>{card.answer}</div>
            </div>
            <div style={S.rateLabel}>理解度を評価してください</div>
            <div style={S.rateRow}>
              {[
                { q: 0, label: "全然", color: "#ef4444" },
                { q: 1, label: "難しい", color: "#f97316" },
                { q: 2, label: "普通", color: "#eab308" },
                { q: 3, label: "簡単", color: "#22c55e" },
              ].map(({ q, label, color }) => (
                <button key={q} style={{ ...S.rateBtn, borderColor: color }}
                  className="rate-btn" onClick={() => handleRate(q)}>
...
function AddView({ onAdd, onCancel }) {
  ...
  return (
    <div style={S.addWrap}>
      ...
      <div style={{ ...S.dropZone, ...(drag ? S.dropZoneActive : {}), ...(image ? S.dropZoneHasImg : {}) }}
        ...
        {image ? (
          <div style={S.previewWrap}>
            ...
            <button style={S.removeImgBtn} onClick={e => { e.stopPropagation(); setImage(null); }}>
          ...
          <div style={S.dropContent}>
            ...
            <div style={S.dropText}>スクリーンショットをドロップ</div>
            <div style={S.dropSub}>またはクリックしてファイルを選択</div>
          </div>
        )}
      ...
      <div style={S.field}>
        <label style={S.label}>補足テキスト（任意）</label>
        <input style={S.input} value={questionText} onChange={e => setQuestionText(e.target.value)}
          placeholder="問題の補足説明..." />
      </div>

      <div style={S.field}>
        <label style={S.label}>答え <span style={{ color: "#ef4444" }}>*</span></label>
        <textarea style={S.textarea} value={answer} onChange={e => setAnswer(e.target.value)}
          placeholder="答えを入力してください..." rows={4} />
      </div>

      <div style={S.addActions}>
        <button style={S.cancelBtn} onClick={onCancel}>キャンセル</button>
        <button
          style={{ ...S.submitBtn, ...(!answer.trim() ? S.submitDisabled : {}) }}
          onClick={() => answer.trim() && onAdd({ questionImage: image, questionText, answer: answer.trim() })}
  ...
  return (
    <div style={S.manageWrap}>
      <div style={S.manageHeader}>
        <input style={{ ...S.input, flex: 1, margin: 0 }} value={search}
          onChange={e => setSearch(e.target.value)} placeholder="🔍 検索..." />
        <div style={{ display: "flex", gap: 6 }}>
          {selected.size > 0 && (
            <button style={S.dangerBtn} onClick={() => setConfirm("selected")}>
              <Icon d={icons.trash} size={13} /> ({selected.size})
            </button>
          )}
          {cards.length > 0 && (
            <button style={S.dangerBtnGhost} onClick={() => setConfirm("all")}>全削除</button>
          )}
        </div>
      </div>
        ...
        <div style={S.selectAllRow}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
            <input type="checkbox"
              ...
              onChange={() => selected.size === filtered.length
                ? setSelected(new Set())
                : setSelected(new Set(filtered.map(c => c.id)))}
              style={{ accentColor: "#f59e0b" }} />
            <span style={{ fontSize: 12, color: "#94a3b8" }}>全て選択</span>
          </label>
          <span style={{ fontSize: 12, color: "#64748b" }}>{filtered.length}枚</span>
        </div>
      )}

      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", color: "#475569", padding: "48px 0", fontSize: 14 }}>
          {search ? "検索結果なし" : "カードがありません"}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(c => {
            ...
            return (
              <div key={c.id}
                style={{ ...S.cardItem, ...(selected.has(c.id) ? S.cardItemSelected : {}) }}
                className="card-item">
                <input type="checkbox" checked={selected.has(c.id)}
                  onChange={() => toggleSelect(c.id)} style={{ accentColor: "#f59e0b", flexShrink: 0 }} />
                <div style={S.cardItemThumb}>
                  {c.questionImage
                    ? <img src={c.questionImage} style={S.thumbImgMd} alt="" />
                    : <div style={S.noThumb}><Icon d={icons.img} size={16} /></div>}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={S.cardItemAnswer}>{c.answer.slice(0, 40)}{c.answer.length > 40 ? "..." : ""}</div>
                  <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 4 }}>
                    <span style={{ ...S.metaChip, ...(due ? S.metaChipDue : S.metaChipOk) }}>
                      {due ? "今日" : `${days}日後`}
                    </span>
                    <span style={S.metaText}>間隔: {c.interval}日</span>
                    <span style={S.metaText}>×{c.repetitions}回</span>
                  </div>
                </div>
                <button style={S.itemDeleteBtn} className="item-del" onClick={() => onDelete(c.id)}>
      ...
      {confirm && (
        <div style={S.overlay}>
          <div style={S.dialog} className="card-appear">
            <div style={S.dialogTitle}>
              {confirm === "all" ? "全てのカードを削除しますか？" : `${selected.size}件を削除しますか？`}
            </div>
            <div style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>この操作は取り消せません</div>
            <div style={{ display: "flex", gap: 10 }}>
              <button style={S.cancelBtn} onClick={() => setConfirm(null)}>キャンセル</button>
              <button style={S.confirmDeleteBtn} onClick={() => {
                if (confirm === "all") onDeleteAll(); else onDeleteSelected();
                setConfirm(null);
              }}>削除する</button>
            </div>
          </div>
...
const S = {
  ...
  rateRow: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, padding: "0 16px 20px" },
  rateBtn: { background: "#0f172a", border: "1.5px solid", borderRadius: 10, padding: "10px 4px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 },
  rateBtnLabel: { fontSize: 13, fontWeight: 700 },
  rateBtnSub: { fontSize: 10, color: "#475569" },
